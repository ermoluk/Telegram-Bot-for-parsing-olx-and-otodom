...
def send_message(result_array, user_id):
    for i in range(len(result_array)):
        ...
        if i > 1 and i < 5:
            ...
            first_image = soup.find('img', {'class': 'css-1bmvjcs'}).get('src')
...
def main():
    while True:
        ...
        for user_key, user_value in users.items():
            urls_users = user_value['avto_name']
            city = user_value['city']
            min_price = user_value['price_min']
            max_price = user_value['price_max']
            r = user_value['rooms']
            users_new_link = ferst_pars(city, min_price, max_price, r)

            result_array = [x for x in users_new_link if x not in urls_users]
            ...
            if result_array == []:
                ...
                users_new = json_upload()
                users_old = json_upload()

                users_new[user_key]['avto_name'] = result_array

                def update_or_add( users_old, key, users_new):
                    if key in users_old:
                        # Обновление существующего ключа
                        users_old[key].update(users_new)
                    else:
                        # Добавление нового ключа
                        users_old[key] = users_new

                for key, data in users_new.items():
                    update_or_add(users_old, key, data)
